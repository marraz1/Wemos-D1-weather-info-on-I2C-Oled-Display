...
void setup() {
 ...
 // WiFi.begin(ssid, password); // Connecting to the WiFi
  ...
  { //Wait till we are connected
   ...
   /* if(WiFi.status() != WL_CONNECTED && wifitry == 5)
    {
     // WiFi.end();
      ssid = "50Butas ";
      ...
      Serial.print(ssid);
      Serial.print( " Pass" );
      Serial.println(passwordh);     
    }
    else if(WiFi.status() != WL_CONNECTED && wifitry == 0)
    {
      //WiFi.end();
      ssid = "GUESTS";
      WiFi.begin(ssidh, password); // Connecting to the WiFi
      Serial.print(ssid);
      Serial.print( " Pass " );
...
void loop()
{
    ...
    { //Check the current connection status
      ...
      http.addHeader("auth-key", "0b36e1cd9cc42d9479648db073403c94d64a491d4a6e81f0fc03a97cd5843bf1");   //Insert your Auth-key here in quotes (as String)
      ...
      { //Check for the returning code
        ...
        char c[] = "}";
        String payload = "";
        http.getString().toCharArray(a, 500);
        for (int i = 0; i < 500; i++) {
          //Serial.println(a[i]);
          if (a[i] == '[') 
          {        
          ...
          {
            //Serial.println("i");
            //payload = String(payload + a[i]);
            //ajson[i] = a[i];
            alenght = i;
            //Serial.println(alenght);
          }
        }

        for (int j = 0; j < alenght-12; j++) {
          payload = String(payload + a[j]);
        }
        ...
        JsonObject Body_Data = root["live_data"];
        if (!root.success()) 
        {
        ...
        String new_cases = Body_Data["cases"]["new"];
        int active_cases = Body_Data["cases"]["active"];
        int critical_cases = Body_Data["cases"]["critical"];
        int recovered_cases = Body_Data["cases"]["recovered"];
        int 1M_pop_cases = Body_Data["cases"]["1M_pop"];
        int total_cases = Body_Data["cases"]["total"];
        int deaths_new_cases = Body_Data["deaths"]["new"];
        int total_deaths_cases = Body_Data["deaths"]["total"];
        Serial.println(new_cases);
        Serial.println(active_cases);
        Serial.println(critical_cases);
        Serial.println(recovered_cases);
        Serial.println(1M_pop_cases);
        Serial.println(total_cases);
        Serial.println(deaths_new_cases);
      ...
      http.end(); //Free the resources
        ...
        String new_cases = root["live_data"]["cases"]["new"];
        int active_cases = root["live_data"]["cases"]["active"];
        int critical_cases = root["live_data"]["cases"]["critical"];
        int recovered_cases = root["live_data"]["cases"]["recovered"];
        int M_pop_cases = root["live_data"]["cases"]["1M_pop"];
        int total_cases = root["live_data"]["cases"]["total"];
        int deaths_new_cases = root["live_data"]["deaths"]["new"];
        int total_deaths_cases = root["live_data"]["deaths"]["total"];
        Serial.println(new_cases);
        Serial.println(active_cases);
        Serial.println(critical_cases);
        Serial.println(recovered_cases);
        Serial.println(M_pop_cases);
        Serial.println(total_cases);
        Serial.println(deaths_new_cases);
        Serial.println(total_deaths_cases);
